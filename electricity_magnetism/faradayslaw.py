...
def animation_action(b, dt, prev_dt):
    if b.text == 'Pause':
        prev_dt[0] = dt[0]
        dt[0] = 0
        b.text = 'Play'
...
def run():
    ...
    # Mutable containers for closure modification
    ...
    PauseButton = button(text='Pause', bind=lambda b: animation_action(b, dt, prev_dt))

    while True:
        rate(0.5 / (dt[0] if dt[0] > 0 else 0.01)) 
        if PauseButton.text == 'Pause':
            magnetic_field.axis = setup_magnet_field(time)  # Update magnetic field vector.
            # Update electric field vectors.
            if dt[0] > 0:
                BDeriv = (setup_magnet_field(time + dt[0]) - setup_magnet_field(time - dt[0])) / (2 * dt[0])
